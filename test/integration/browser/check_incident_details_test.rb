...
class CheckIncidentDetailsTest < ActionDispatch::IntegrationTest
  ...
  test 'Check the title of #details-modal'  do 
    ...
    is_css_in_page? '#details-modal' 
    is_text_in_section?('#details-modal', "Current Status of  #{@sys_name}")
    is_image_in_section?('#details-modal', 'amber.png', 1)
  end

  test 'Check the contents of #details-modal'  do 
    # Check the contents of the <div> .main-section
    is_text_in_section? '#incident-section', "Incidents Occuring in the last 24 hours"
    is_text_in_section? '#incident-section', @incident_detail_desc
    is_text_in_section? '#history-section', "Chronological List of Historic Incidents"
    is_text_in_section? '#history-section', @incident_hist_desc 
  end
