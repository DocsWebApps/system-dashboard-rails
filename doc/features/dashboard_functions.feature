Feature: Check the dashboard functions correctly
	...
	I want the statuses and indicators for the various systems to automatically update on each view according to the rules set out below
		...
		2. If a P1 incident(s) is(are) currently running on a system, its status should show a red smiley and details of the incident should be displayed in the #incident-details section of the 'More Information' page	
		
		3. If a P2 incident(s) is(are) currently running on a system, its status should show an amber smiley and details of the incident should be displayed in the #incident-details section of the 'More Information' page

		4. If both a P1(s) and a P2(s) incident(s) is(are) currently running on a system, its status should show a red smiley and details of all P1/P2 incidents should be displayed in the #incident-details section of the 'More Information' page
		
		5. If a system has had an Open P1 or P2 incident(s) in the last 24 hours, which is(are) now closed, its status should show a green smiley and its indicator should display either 'X Incident(s) in the last 24 hours' in red, where 'X' is the number of incidents in the last 24 hours. The details of the incident should be visible in the #incident-details section with a status of 'Closed'.
		...
		Both should be displayed in green, but if any old incidents are recorded as 'D', they should not be included in this logic.
		
		7. Any closed incident over 24 hours old should not be visible in the #incident-details section, but should be visible in the #incident-history section
...
Background: Assume the following data exists in the system
	...
	kirk has no P1/P2 incidents running and one other incident in the database that was downgraded
	spock has a P2 incident running now, but has no other incidents recorded in the database
	bones has no P1/P2 incidents running now but had a P1 in the last 24 hours that is closed and no other incidents recorded in the database
	sulu has a P1 running now, none in the last 24 hours, but it has an old closed incident 4 days old
